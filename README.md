...
### 전체 시스템 플로우
```
사용자 질문 → Cloud Run → Google VM (임베딩) → Google VM (ChromaDB) → 검색 결과 → OpenAI → 답변 생성
```

### 상세 아키텍처
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────────────────┐
│   Frontend      │────│   Cloud Run     │────│     Google Cloud VM         │
│   (HTML/JS)     │    │   (FastAPI)     │    │  ┌─────────────────────────┐ │
...
### 처리 과정
1. **사용자 입력**: 웹 인터페이스를 통한 법률 질문 입력
2. **Cloud Run 처리**: FastAPI 서버에서 요청 수신 및 전처리
3. **Google VM 임베딩**: 
   - 질문을 Google Cloud VM의 임베딩 서비스로 전송
   - ko-sbert-nli 모델을 통해 벡터 변환
4. **ChromaDB 검색**: 
   - 동일 VM 내 ChromaDB에서 유사 문서 검색
   - 5개 컬렉션에서 병렬 검색 수행
5. **컨텍스트 구성**: 검색된 법률 문서들을 컨텍스트로 구성
6. **OpenAI 생성**: GPT-4o-mini로 법률 자문 생성
7. **스트리밍 응답**: 실시간으로 사용자에게 답변 전송
8. **로깅**: BigQuery에 상호작용 데이터 저장

### 배포 환경
- **Cloud Run**: 메인 FastAPI 서버 (서버리스 컨테이너)
- **Google Cloud VM**: ChromaDB + 임베딩 서비스 호스팅
  - VM 사양: 법률 문서 벡터 DB 및 임베딩 처리 최적화
  - 네트워크: 내부 통신으로 빠른 검색 성능 보장
